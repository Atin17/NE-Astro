...
def train_actor_critic(
    ...
    for episode in range(num_episodes):
        ...
        # Create env
        # e.g. if we want 50% go, do go_tone=np.random.rand()<0.5
        go_tone=np.random.rand() < go_prob
        env = env_class(go_tone, step_ms=step_ms)
        ...
        while not done:
            ...
            if np.random.rand() < eps and env.in_response_window and go_tone:
        ...
        # Here, let's guess we have 'env.go_tone' and 'env.has_pressed' as typical:
...
def main():
    ...
    success_history = train_actor_critic(
        env_class=MouseBehaviorEnvTemporalReward,  # pass environment
        go_prob=0.5,
    ...
    plt.xlabel("Trials")
    plt.ylabel("Success")
    plt.title("Actor-Critic with GRU on Temporal Reward Env")
    plt.legend()
    plt.grid(True)
    plt.show()
